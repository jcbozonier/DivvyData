...
def update_appropriate(the_futures, current_date, station_id, was_bike_available, was_dock_available):
	updates = [{'index_to_update': i, 'time_to_log': subtract_from_date(current_date, i)} for i in xrange(0,31)]
	...
	for event in events:
		...
		for station_data in event['stationBeanList']:
			...
			if not (key in futures):
				futures[key] = {
					'future_bike_availability': [0]*31,
					'future_dock_availability': [0]*31,
					'total_count': 0
				}
			update_appropriate(futures, event_date, station_data['id'], station_data['availableBikes'] > 0, station_data['availableDocks'] > 0)

csv_ready_future_availability = [{'station_id': item[0][0], 'hour':item[0][1], 'minute':item[0][2], 'weekday':item[0][3], 'hour_id': item[0][1] + 24*item[0][3], 'minute_id': item[0][1] + 24*60*item[0][3] + 60*item[0][2], 'future_bike_availability':item[1]['future_bike_availability'], 'future_dock_availability':item[1]['future_dock_availability']} for item in futures.items()]
